...
### Skills
- `generate` — 対話型ウィザード。商材情報をヒアリングし、`.team-config.json` + プラグインを生成
...
### 再生成（設定変更後）
```
/forge-generate path/to/.team-config.json
```
...
`{product}-context-stocker` という名前のプラグインが生成される。5つのスキル（deal/admin/log/doc/knowledge）と8つのコマンドで構成される。
...
| `{pre}-log <サブコマンド>` | 活動ログ（daily / weekly / report） |
