...
└── templates/                          # ★生成プラグインのテンプレート群
    ...
    └── commands/                       # → 生成時 commands/{pre}-{group}-{action}.md になる
        ├── admin/  (8コマンド: backlog, index, kpi-set, migrate, okr-set, setup, slack, stale)
        ├── deal/   (2コマンド: load, save)
        ├── doc/    (6コマンド: config, estimate, hearing, prep, proposal, testcases)
        ├── knowledge/ (2コマンド: save, search)
        └── log/    (2コマンド: daily, report)
```
...
各コマンドテンプレートは `「{セクション名}」セクションの手順に従って実行` という形式でdealスキル内のセクションを参照する。
**参照先のセクションがdealスキルテンプレート内に実際に存在するか**を確認する。

| コマンド | 呼び出すスキル | 参照セクション名 |
|---------|-------------|---------------|
| admin-backlog | deal | Backlogプロジェクト設定管理 |
| admin-index | deal | INDEX再構築 |
| admin-kpi-set | deal | 参照ファイル kpi-format.md |
| admin-migrate | deal | フォーマットマイグレーション |
| admin-okr-set | deal | 参照ファイル okr-format.md |
| admin-setup | deal | セットアップ検証 |
| admin-slack | deal | Slackチャンネル設定管理 |
| admin-stale | deal | 鮮度管理 |
| deal-load | deal | コンテキスト復元 |
| deal-save | deal | コンテキスト保存 |
| doc-config | deal | 設定設計書作成 |
| doc-estimate | deal | 概算見積もり作成 |
| doc-hearing | deal | ヒアリングシート作成 |
| doc-prep | deal | 商談準備資料作成 |
| doc-proposal | deal | 提案書ドラフト作成 |
| doc-testcases | deal | ガイドライン参照による業務ドキュメント生成 |
| knowledge-save | knowledge | ナレッジ保存 |
| knowledge-search | knowledge | ナレッジ検索 |
| log-daily | deal | 日次活動ログ |
| log-report | deal | 活動レポート生成 |
...
1. **スキル名**: `{{product_prefix}}-deal`（contextではない）
2. **コマンド参照**: `{{plugin_name}}:{{product_prefix}}-deal`（contextではない）
3. **knowledgeスキル参照**: `{{plugin_name}}:{{product_prefix}}-knowledge`
4. **コマンド名パターン**: `{{product_prefix}}-{group}-{action}`
...
dealスキルテンプレート末尾の「詳細リファレンス」セクションに記載された `references/*.md` ファイルが、テンプレートディレクトリに実際に存在するか。

### チェック10: ストレージアダプタの操作コマンド対応

dealスキルテンプレートで使われる `{{storage_*_cmd}}` 系変数が、両方のストレージアダプタ（backlog-wiki.md, obsidian-vault.md）で適切に定義されているか。
...
- 特にdoc系コマンド（config, estimate, hearing, prep, proposal, testcases）は、「ガイドライン参照による業務ドキュメント生成」で包括されている可能性があるが、コマンドテンプレートの記述と実際のスキルセクション名が一致するか厳密に確認
